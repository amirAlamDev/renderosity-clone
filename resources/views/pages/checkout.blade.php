...
@section('styles')
<style>
	body { -webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility; font-family: 'Noto Sans', sans-serif; letter-spacing: 0px; font-size: 14px; color: #2e3139 !important; font-weight: 400; line-height: 26px; }
	h1, h2, h3, h4, h5, h6 { letter-spacing: 0px; font-weight: 400; color: #1c1e22 !important; margin: 0px 0px 15px 0px; font-family: 'Noto Sans', sans-serif; }
	...
	p { margin: 0 0 20px; line-height: 1.8; color: #1c1e22 !important;}
...
@if($errors->any())
	<h4 id="transaction-failed" class="alert alert-danger" style="display: none;">{{$errors->first()}}</h4>
@endif

@if(session('success'))
	<h4 id="transaction-succeeded" class="alert alert-success" style="display: none;">{{ session('success') }}</h4>
	<script>
		setTimeout(()=>{
...
@section('content')
	<checkout  />
@endsection
	<form method="post" id="paypal-payment-form" action="{{ route('checkout.paypal') }}" style="display: none;">
		@csrf
		<section>
...
window.addEventListener('load', function () {
	...
	braintree.dropin.create({
	  ...
	  elem = document.getElementsByClassName('braintree-heading')[2];
	  elem.parentNode.removeChild(elem);
	...
	var error = document.querySelector('#transaction-failed');
	var success = document.querySelector('#transaction-succeeded');
	
	document.querySelector('.paypal-payment').appendChild(form);
	form.style.display = 'block';
	
	if (document.querySelector('#transaction-failed')) {
		document.querySelector('.box').insertBefore(error, document.querySelector('.box').firstChild);
		error.style.display = 'block';
	} else if (document.querySelector('#transaction-succeeded')) {
		document.querySelector('.box').insertBefore(success, document.querySelector('.box').firstChild);
	...
	firstName1.addEventListener('change', function() { form.firstName.value = firstName1.value; });
	lastName1.addEventListener('change', function() { form.lastName.value = lastName1.value; });
	email1.addEventListener('change', function() { form.email.value = email1.value; });
	address1.addEventListener('change', function() { form.address.value = address1.value; });
	zipCode1.addEventListener('change', function() { form.zipCode.value = zipCode1.value; });
	city1.addEventListener('change', function() { form.city.value = city1.value; });
	country1.addEventListener('change', function() { form.country.value = country1.value; });
});
</script>